...
//#####################################################################################################
  ...
  static inline uint16_t raw16_to_raw10(int16_t raw16) {
    ...
    //SERIAL_ECHOPGM(">> raw16_to_raw10 input raw16 = "); SERIAL_ECHOLN(raw16);
    uint16_t mag = raw16 < 0 ? -raw16 : raw16;
    uint16_t raw10 = mag >> 5;        // reduz 16→10 bits
    SERIAL_ECHOPGM("raw16_to_raw10 = "); SERIAL_ECHOLN(raw10);
  ...
  void Temperature::initpcf8574ads1115beds() {
    ...
    SERIAL_ECHOLNPGM("ADS1115 and PCF8574 init OK");
    

    #ifdef SIMULAR_FLUXOADSPCF 
    // DEBUG: simulate conversion for each bed
    raw_adc_t debug_raws[MULTI_BED_COUNT] = { 0, 12000, 24000, 30000};
    SERIAL_ECHOLNPGM(">> Debug conversion RAW->C");
    for (uint8_t b = 0; b < MULTI_BED_COUNT; b++) {
      const raw_adc_t raw16 = debug_raws[b];
      uint16_t raw10 = raw16_to_raw10(raw16);
      celsius_t temp = analog_to_celsius_bed(raw10);
      SERIAL_ECHOPGM(" Bed["); SERIAL_ECHO(b);
      SERIAL_ECHOPGM("] raw16="); SERIAL_ECHO(raw16);
      SERIAL_ECHOPGM("] raw10="); SERIAL_ECHO(raw10);
      SERIAL_ECHOPGM(" celsius= "); SERIAL_ECHOLN(temp);
    }            
    #endif    
  ...
  */
  ...
  void Temperature::update_bed_pwm_pcf8574() {
    ...
    //SERIAL_ECHOPGM(">> update_bed_pwm_pcf8574 pwm_step = ");
    //SERIAL_ECHOLN(pwm_step);

    uint8_t state = 0;
    for (uint8_t b = 0; b < MULTI_BED_COUNT; b++) {            
      //SERIAL_ECHOPGM(" Bed["); SERIAL_ECHO(b);
      //SERIAL_ECHOPGM("] soft_pwm_amount = ");
    ...
    if (err) {
      SERIAL_ECHOPGM("!! PCF8574 write error: ");SERIAL_ECHOLN(err);          
      SERIAL_ECHOLNPGM("Updating pwm pcf8574");
    }
  }  
  ...
  void Temperature::setTargetBed(uint8_t bed, const celsius_t celsius) {
    ...
    //SERIAL_ECHOPGM("setTargetBed bed="); SERIAL_ECHO(bed);
    //SERIAL_ECHOPGM(" target="); SERIAL_ECHOLN(celsius);
    ...
    SERIAL_ECHOLNPGM("Setting specific target bed");
  }
  ...
  void Temperature::set_all_beds_target(const celsius_t celsius) {
      ...
      SERIAL_ECHOLNPGM("Setting all target beds");
  }
  ...
  // Multi-bed: gerencia cada cama individualmente
   ...
   void Temperature::manage_heated_beds(const uint8_t bed, const millis_t &ms) {
    ...
    // 1) Proteção de temperatura máxima
        ...
        if (degBed(bed) > BED_MAXTBED_MAXTEMPEMP) {
          max_temp_error(hid);
        }
    #endif
  ...
  void Temperature::tr_state_machine_t::run(const_celsius_float_t current, const_celsius_float_t target, const heater_id_t heater_id, const uint16_t period_seconds, const celsius_t hysteresis_degc) {
    ...
    */
    ...
    switch (state) {
      ...
      } // fall through // fim do bloco TRStable
...
//#####################################################################################################
        ...
        void Temperature::pause_heaters(const bool p) {
          if (p != paused_for_probing) {
            ...
            if (p) {
              ...
              for (uint8_t b = 0; b < MULTI_BED_COUNT; ++b)  // Timeout imediato em todas as beds
                  heater_idle[ IDLE_INDEX_BED0 + b ].expire();
            }
            else{
                // Reinicia timeout dos hotends
               HOTEND_LOOP() reset_hotend_idle_timer(e);
               // Reinicia timeout de cada cama
               for (uint8_t b = 0; b < MULTI_BED_COUNT; ++b)
               reset_bed_idle_timer(b);
            }
          }
        }
  #elif HAS_HEATED_BED// Fallback single-bed (original)
    ...
    if (p != paused_for_probing) {
      ...
      if (p) {
        HOTEND_LOOP() heater_idle[e].expire();    // Timeout immediately
        heater_idle[IDLE_INDEX_BED].expire(); // Timeout immediately
      }
      else {
        HOTEND_LOOP() reset_hotend_idle_timer(e);
...
void Temperature::readings_ready() {
  ...
  //#####################################################################################################
    ...
    TERN_(HAS_TEMP_ADC_BED, temp_bed.reset());
  #endif
...
void Temperature::isr() {
  ...
  #define WRITE_FAN(n, v) WRITE(FAN##n##_PIN, (v) ^ FAN_INVERTING)
    ...
    if (pwm_count_tmp >= 127) {
      ...
      //#####################################################################################################
          ...
          // 3) Transação única no barramento I²C
          bedPCF.write8(state);
        #else
          // fluxo original para uma cama
    ...
    if (slow_pwm_count == 0) {
      ...
      //#####################################################################################################
          ...
          for (uint8_t b = 0; b < MULTI_BED_COUNT; ++b) {
            ...
            if (temp_bed[b].soft_pwm_amount > 0 && soft_pwm_bed[b].ready(true))
                state |=  _BV(BED0_PCF_BIT + b);
  ...
  switch (adc_sensor_state) {
    ...
    //########################          TCC LUCAS          ################################################
    //#####################################################################################################
    #if HAS_TEMP_ADC_BED && DISABLED(ENABLE_MULTI_HEATED_BEDS)
  ...
  //#####################################################################################################
    ...
    bool Temperature::wait_for_hotend(const uint8_t target_extruder, const bool no_wait_for_cooling/*=true*/
      ...
      bool Temperature::wait_for_bed(
        ...
        // --- Early exit se nada para aguardar ---
        if (!( isHeatingBed(bed) || (!no_wait_for_cooling && isCoolingBed(bed)) ))
          return false;

        // --- Mensagens de início, por cama ---
        ...
        do {
          // Atualiza alvo e sentido de controle
          celsius_t new_target = degTargetBed(bed);
          if (target_temp != new_target) {
          ...
          #if TEMP_BED_RESIDENCY_TIME > 0
            const celsius_float_t diff = fabs(target_temp - temp);
            if (!residency_start_ms) {
          ...
          // Proteção contra “wait forever” no cooling
          if (wants_to_cool) {
            if (!next_cool_check_ms || ELAPSED(now, next_cool_check_ms)) {
      ...
      #undef TEMP_BED_CONDITIONS //wait_for_bed
      ...
      // Máscara de camas ainda “pendentes”
        uint8_t pending = (1 << MULTI_BED_COUNT) - 1;

        // Temporizadores auxiliares
        millis_t now         = 0,
                next_temp_ms = 0;

        // Loop principal: termina quando pending == 0 ou cancelado
        while (pending && wait_for_heatup) {
          now = millis();

          // Imprime status geral a cada 1s
          if (ELAPSED(now, next_temp_ms)) {
            next_temp_ms = now + 1000UL;
            print_heater_states(active_extruder);
            SERIAL_EOL();
          ...
          // Processa cama a cama
          for (uint8_t b = 0; b < MULTI_BED_COUNT; ++b) {
            if (!(pending & (1 << b))) continue;        // já finalizada

            // Atualiza alvo e sentido
            celsius_t new_t = degTargetBed(b);
            if (new_t != target_temp[b]) {
              target_temp[b]   = new_t;
            ...
            // Leitura atual
            celsius_float_t cur = degBed(b);

            // LEDs
            #if ENABLED(PRINTER_EVENT_LEDS)
              if (!wants_to_cool[b])
                printerEventLEDs.onBedHeating(
                  start_temp[b], cur, target_temp[b]
                );
            #endif

            // Residência (se habilitado)
            #if TEMP_BED_RESIDENCY_TIME > 0
              const celsius_float_t diff = ABS(target_temp[b] - cur);
              if (!residency_start_ms[b]) {
                if (diff < TEMP_BED_WINDOW)
                  residency_start_ms[b] = now + (first_loop[b]
                    ? SEC_TO_MS(TEMP_BED_RESIDENCY_TIME)/3
                    : 0);
              }
              else if (diff > TEMP_BED_HYSTERESIS) {
            ...
            // Proteção cooling slope
            if (wants_to_cool[b]) {
              if (!next_cool_check_ms[b] || ELAPSED(now, next_cool_check_ms[b])) {
                if (old_temp[b] - cur < float(MIN_COOLING_SLOPE_DEG_BED)) {
                  wait_for_heatup = false;
                  break;
                }
            ...
            // Verifica se esta cama já concluiu a espera
            bool done;
            #if TEMP_BED_RESIDENCY_TIME > 0
              // saiu da janela de espera/residência?
              done = residency_start_ms[b]
                && !PENDING(now, residency_start_ms[b] + SEC_TO_MS(TEMP_BED_RESIDENCY_TIME));
            #else
              done = wants_to_cool[b]
                ? !isCoolingBed(b)
                : !isHeatingBed(b);
            #endif
      ...
      void Temperature::wait_for_bed_heating(uint8_t bed){ //MultiBed {
        if ( isHeatingBed(bed) ) {
          ...
          // LCD_MESSAGE(MSG_BED_HEATING " #" + String(bed+1));
          LCD_MESSAGE(MSG_BED_HEATING);
          wait_for_bed(bed);            // chama a versão parametrizada
          ui.reset_status();
        }
      }
